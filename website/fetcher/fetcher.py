...
def fetch_main_branch_revisions():
    ...
    # cutoff hash.
    latest_main_branch_revision = Revision.objects.filter(branch=main_branch_name)
    if latest_main_branch_revision.exists():
        cutoff_commit_hash = latest_main_branch_revision.latest("date_added").hash
    ...
    for hash in reversed(revisions_to_create_hashes):
        ...
        if (
            "llvm/lib/Target/SPIRV" in changed_files
            or "llvm/test/CodeGen/SPIRV" in changed_files
        ):
            skip = False
        else:
            skip = True
...
def fetch_staging_revisions():
    ...
    for branch_and_subject in remote_branches_list:
        ...
        hash = (
            subprocess.check_output(
                ["git", "rev-parse", branch_name], cwd=repository_path
            )
            .decode("utf-8")
            .splitlines()[0]
        )
